...
class DAMRS(GeneralRecommender):
    def __init__(self, config, dataloader):
        super(DAMRS, self).__init__(config, dataloader)
        ...
        # load dataset info (MRS: use dataloader.dataset)
        ...
        if self.visual_feat is not None:
            self.image_embedding = nn.Embedding.from_pretrained(self.visual_feat, freeze=True)
            self.image_trs = nn.Linear(self.visual_feat.shape[1], self.embedding_dim)
        if self.textual_feat is not None:
            self.text_embedding = nn.Embedding.from_pretrained(self.textual_feat, freeze=True)
            self.text_trs = nn.Linear(self.textual_feat.shape[1], self.embedding_dim)

        # Use cache manager for item_graph_dict
        cache_manager = GraphCacheManager(config['data_path'], config['dataset'])
        cache_dir = cache_manager.get_model_cache_dir('DAMRS')
        
        item_graph_file = os.path.join(cache_dir, 'item_graph_dict.pt')
        
        # Load item_graph_dict (same as MMRec-tmp, load from file)
        if os.path.exists(item_graph_file):
            self.item_graph_dict = torch.load(item_graph_file)
            print(f">>>>Loaded item_graph_dict from: {item_graph_file}")
        else:
            # Build item-item graph using dataset (MRS: use dataloader.dataset)
            from collections import defaultdict
            train_interactions = dataloader.dataset.inter_matrix(form='coo').astype(np.float32)
            
            # Build edge_dict: item -> set of users who interacted with it
            edge_dict = defaultdict(set)
            user_ids, item_ids = train_interactions.nonzero()
            for user_id, item_id in zip(user_ids, item_ids):
                edge_dict[item_id].add(user_id)
            
            item_graph_matrix = np.zeros((self.n_items, self.n_items))
            key_list = sorted(list(edge_dict.keys()))
            
            print('Building item-item graph...')
            for head_idx in range(len(key_list)):
                if head_idx % 1000 == 0:
                    print(f'  Processing item {head_idx}/{len(key_list)}...')
                head_key = key_list[head_idx]
                for rear_idx in range(head_idx+1, len(key_list)):
                    rear_key = key_list[rear_idx]
                    user_head = edge_dict[head_key]
                    user_rear = edge_dict[rear_key]
                    inter_len = len(user_head.intersection(user_rear))
                    if inter_len > 0:
                        item_graph_matrix[head_key][rear_key] = inter_len
                        item_graph_matrix[rear_key][head_key] = inter_len
            
            self.item_graph_dict = {}
            import torch as torch_lib
            for i in range(self.n_items):
                item_num_i = len(torch_lib.nonzero(torch_lib.from_numpy(item_graph_matrix[i])))
                if item_num_i <= 200:
                    item_i = torch_lib.topk(torch_lib.from_numpy(item_graph_matrix[i]), int(item_num_i))
                else:
                    item_i = torch_lib.topk(torch_lib.from_numpy(item_graph_matrix[i]), 200)
                edge_list_i = item_i.indices.numpy().tolist()
                edge_list_j = item_i.values.numpy().tolist()
                self.item_graph_dict[i] = [edge_list_i, edge_list_j]
            
            torch.save(self.item_graph_dict, item_graph_file)
            print('Built item-item graph')
