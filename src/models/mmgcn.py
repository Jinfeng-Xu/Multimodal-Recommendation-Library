...
class MMGCN(GeneralRecommender):
    def __init__(self, config, dataloader):
        ...
        if self.visual_feat is not None:
            self.v_gcn = GCN(self.edge_index, batch_size, num_user, num_item, self.visual_feat.size(1), dim_x, self.aggr_mode,
                             self.concate, num_layer=num_layer, has_id=has_id, dim_latent=256, device=self.device)
            self.num_modal += 1

        if self.textual_feat is not None:
            self.t_gcn = GCN(self.edge_index, batch_size, num_user, num_item, self.textual_feat.size(1), dim_x,
    ...
    def forward(self):
        representation = None
        if self.visual_feat is not None:
            representation = self.v_gcn(self.visual_feat, self.id_embedding)
        if self.textual_feat is not None:
            if representation is None:
                representation = self.t_gcn(self.textual_feat, self.id_embedding)
            else:
                representation += self.t_gcn(self.textual_feat, self.id_embedding)
