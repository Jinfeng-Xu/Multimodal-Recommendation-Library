...
class PGL(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        cache_dir = cache_manager.get_model_cache_dir('PGL')
        mm_adj_file = os.path.join(cache_dir, 'mm_adj_freedomdsp_{}_{}.pt'.format(self.knn_k, int(10 * self.mm_image_weight)))
        ...
        if os.path.exists(mm_adj_file):
            ...
            if self.visual_feat is not None and self.textual_feat is not None:
                ...
                del text_adj
                del image_adj
            cache_manager.save_graph('PGL', 'mm_adj', self.mm_adj)
    ...
    def global_subgraph_extraction(self, adj):
        norm_adj = adj.tocsc()
        # svds returns k singular values/vectors, need to request embedding_dim
        # svds returns vt transposed compared to sparsesvd
        vt, s, ut = svds(norm_adj, k=self.embedding_dim, return_singular_vectors='both')
        # Transpose ut to match sparsesvd output format
        ut = ut.T
