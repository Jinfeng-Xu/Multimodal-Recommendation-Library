...
r"""
MENTOR: Multi-level Self-supervised Learning for Multimodal Recommendation, AAAI, 2025
"""
...
class MENTOR(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        # Use cache manager for user-graph and mm_adj (MRS framework feature)
        cache_manager = GraphCacheManager(config['data_path'], config['dataset'])
        cache_dir = cache_manager.get_model_cache_dir('MENTOR')
        
        user_graph_file = os.path.join(cache_dir, 'user_graph_dict.pt')
        mm_adj_file = os.path.join(cache_dir, 'mm_adj_{}.pt'.format(self.knn_k))
        
        # Load or build user-user graph (exactly  MMRec-tmp preprocessing)
        if os.path.exists(user_graph_file):
            self.user_graph_dict = torch.load(user_graph_file)
            # Ensure keys are integers (torch may save them as strings)
            if self.user_graph_dict:
                first_key = next(iter(self.user_graph_dict.keys()))
                if isinstance(first_key, str):
                    self.user_graph_dict = {int(k): v for k, v in self.user_graph_dict.items()}
        else:
            # Build user-user graph exactly as MMRec-tmp preprocessing/dualgnn-gen-u-u-matrix.py
            from collections import defaultdict
            
            # Get training interactions only ( line 72-73 of preprocessing script)
            train_interactions = dataset.inter_matrix(form='coo').astype(np.float32)
            
            # Build edge_dict ( line 15-19)
            edge_dict = defaultdict(set)
            user_edges = zip(*train_interactions.nonzero())
            for user, item in user_edges:
                edge_dict[user].add(item)
            
            # Build user_graph_matrix ( line 21-42)
            min_user = 0
            num_user = self.n_users
            ...
            for head in range(len(key_list)):
                if head % 1000 == 0:
                    print(f'  Processing user {head}/{len(key_list)}...')
            ...
            # Build user_graph_dict ( line 82-103)
            self.user_graph_dict = {}
            for i in range(num_user):
                user_num_i = len(torch.nonzero(user_graph_matrix[i]))
                if user_num_i <= 200:
                    user_i = torch.topk(user_graph_matrix[i], int(user_num_i))
                else:
                ...
                self.user_graph_dict[i] = edge_list
            
            torch.save(self.user_graph_dict, user_graph_file)
            print('Built user-user graph')
        ...
        if os.path.exists(mm_adj_file):
            ...
            if self.visual_feat is not None and self.textual_feat is not None:
                ...
                del text_adj
                del image_adj
            cache_manager.save_graph('MENTOR', 'mm_adj', self.mm_adj)
    ...
    def forward(self, interaction):
        ...
        self.v_rep_n1, _ = self.v_gcn_n1(self.edge_index_dropv, self.edge_index, self.visual_feat, perturbed=True)
        self.t_rep_n1, _ = self.t_gcn_n1(self.edge_index_dropt, self.edge_index, self.textual_feat, perturbed=True)
        self.v_rep_n2, _ = self.v_gcn_n2(self.edge_index_dropv, self.edge_index, self.visual_feat, perturbed=True)
        self.t_rep_n2, _ = self.t_gcn_n2(self.edge_index_dropt, self.edge_index, self.textual_feat, perturbed=True)
