...
class LGMRec(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        if self.visual_feat is not None:
            ...
            self.item_image_trs = nn.Parameter(nn.init.xavier_uniform_(torch.zeros(self.visual_feat.shape[1], self.feat_embed_dim)))
            self.v_hyper = nn.Parameter(nn.init.xavier_uniform_(torch.zeros(self.visual_feat.shape[1], self.hyper_num)))
        if self.textual_feat is not None:
            self.text_embedding = nn.Embedding.from_pretrained(self.textual_feat, freeze=True)
            self.item_text_trs = nn.Parameter(nn.init.xavier_uniform_(torch.zeros(self.textual_feat.shape[1], self.feat_embed_dim)))
            self.t_hyper = nn.Parameter(nn.init.xavier_uniform_(torch.zeros(self.textual_feat.shape[1], self.hyper_num)))
    ...
    def ssl_triple_loss(self, emb1, emb2, all_emb):
        ...
        ssl_loss = -torch.log(pos_score / ttl_score).mean()  # Use mean instead of sum
        return ssl_loss
