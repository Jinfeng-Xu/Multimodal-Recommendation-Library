...
r"""
Enhancing Dyadic Relations with Homogeneous Graphs for Multimodal Recommendation, ECAI, 2023
"""
...
class DRAGON(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        cache_dir = cache_manager.get_model_cache_dir('DRAGON')
        ...
        if os.path.exists(user_graph_file):
            print('Loading user_graph_dict from {}'.format(user_graph_file))
            self.user_graph_dict = torch.load(user_graph_file)
        else:
            print('Building user-user graph...')
            train_interactions = dataset.inter_matrix(form='coo').astype(np.float32)
            user_item_matrix = train_interactions.tocsr()
            ...
            user_user_sim = user_item_normalized.dot(user_item_normalized.T)
            
            self.user_graph_dict = {}
            for u in range(self.n_users):
                sim_scores = user_user_sim.getrow(u).toarray().flatten()
                sim_scores[u] = -np.inf
                top_k_neighbors = np.argsort(sim_scores)[-self.k:]
                self.user_graph_dict[u] = [top_k_neighbors.tolist(), [1.0] * len(top_k_neighbors)]
            
            os.makedirs(os.path.dirname(user_graph_file), exist_ok=True)
            torch.save(self.user_graph_dict, user_graph_file)
            print('Built and saved user-user graph to {}'.format(user_graph_file))
        ...
        if os.path.exists(mm_adj_file):
            print('Loading mm_adj from {}'.format(mm_adj_file))
            ...
            print('Saved mm_adj to {}'.format(mm_adj_file))
        ...
        if self.visual_feat is not None:
            ...
            self.v_gcn = GCN(self.dataset, batch_size, num_user, num_item, dim_x, self.aggr_mode,
                             ...
                             device=self.device, features=self.visual_feat)  # 256)
        if self.textual_feat is not None:
