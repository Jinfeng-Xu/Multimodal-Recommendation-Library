...
class SMORE(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        # Use cache manager for adjacency matrices
        cache_manager = GraphCacheManager(config['data_path'], config['dataset'])
        cache_dir = cache_manager.get_model_cache_dir('SMORE')
        
        image_adj_file = os.path.join(cache_dir, 'image_adj_{}_{}.pt'.format(self.image_knn_k, self.sparse))
        text_adj_file = os.path.join(cache_dir, 'text_adj_{}_{}.pt'.format(self.text_knn_k, self.sparse))
