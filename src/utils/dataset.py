...
class RecDataset(object):
    ...
    def split(self):
        """
        Split data, filter cold-start users in validation/test sets
        Ensure users in validation/test sets have appeared in training set
        """
        dfs = []
        ...
        # Standard behavior: filter cold-start users in validation/test sets
        if self.config.get('filter_out_cod_start_users', True):
            self.logger.info('Filtering out cold-start users in validation/test sets...')
            ...
            for i in [1, 2]:
                ...
                dropped_count = dropped_inter.sum()
                dfs[i].drop(dfs[i].index[dropped_inter], inplace=True)
                self.logger.info(f'Set {i} ({"val" if i==1 else "test"}): removed {dropped_count} interactions from cold-start users')
    ...
    def inter_matrix(self, form='coo', dtype=np.float32):
        ...
        src = self.df[self.uid_field].values
        tgt = self.df[self.iid_field].values
        ...
        if form == 'coo':
            ...
            return mat.tocsr().astype(dtype)
        else:
            raise NotImplementedError(f'Unknown format {form}')
